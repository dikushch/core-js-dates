...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const t = new Date(dateEnd).getTime() - new Date(dateStart).getTime();
  return t / 86400000 + 1;
}
...
function formatDate(date) {
  ...
  return `${month}/${day}/${year}, ${hour}:${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')} ${pmam}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  const weekends = days.filter((i) => i === 0 || i === 6);
